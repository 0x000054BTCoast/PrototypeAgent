...
import { UISchema, UIComponent, Position } from "@prd2prototype/schema";
...
const inferComponent = (line: string, index: number): UIComponent => {
  ...
  if (text.includes("chart")) return { id, type: "chart", props: { title: line.trim() }, style: {}, children: [] };
  if (text.includes("table")) return { id, type: "table", props: { title: line.trim() }, style: {}, children: [] };
  if (text.includes("button")) return { id, type: "button", props: { label: line.replace(/[-*]/g, "").trim() }, style: {}, children: [] };
  if (text.includes("card")) return { id, type: "card", props: { title: line.trim() }, style: {}, children: [] };
  return { id, type: "text", props: { content: line.replace(/[-*]/g, "").trim() }, style: {}, children: [] };
};
...
export const parsePrdToSchema = (markdown: string): UISchema => {
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith("#")) {
      ...
      if (!currentSection) {
        currentSection = { id: "section_1", name: toSnakeCase(heading || "main"), position: inferPosition(heading), components: [] };
        sections.push(currentSection);
      }
      continue;
    }
  ...
  if (!sections.length) {
    sections.push({
      ...
      name: "main",
      position: "center",
      components: [{ id: "component_1", type: "text", props: { content: "empty_prd" }, style: {}, children: [] }]
    });
  }

  if (!sections.some((section) => section.components.length > 0)) {
    sections[0].components.push({ id: "component_1", type: "text", props: { content: "placeholder" }, style: {}, children: [] });
  }
