...
import { UISchema, UIComponent, Position } from "./types.js";
...
const inferComponent = (line: string, index: number): UIComponent => {
  ...
  if (text.includes("button")) {
    return { id, type: "button", props: { label: line.replace(/[-*]/g, "").trim() }, style: {}, children: [] };
  }
...
if (process.argv[1]?.endsWith("prd-to-json.ts")) {
  const inputPath = path.resolve("input/prd.md");
  const outputPath = path.resolve("output/structure.json");
