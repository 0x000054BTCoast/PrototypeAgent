...
if (process.argv[1]?.endsWith("exporters.ts")) {
  const schema = JSON.parse(fs.readFileSync(path.resolve("output/structure.json"), "utf8")) as UISchema;
  fs.writeFileSync(path.resolve("output/prototype.svg"), `${exportSvg(schema)}\n`);
  fs.writeFileSync(path.resolve("output/preview.html"), `${exportHtml(schema)}\n`);
}
